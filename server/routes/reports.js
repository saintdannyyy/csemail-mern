...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const emailsOpened =
        events?.filter((event) => event.type === "opened").length || 0;
      const emailsClicked =
        events?.filter((event) => event.type === "clicked").length || 0;
      const emailsBounced =
        events?.filter((event) => event.type === "bounced").length || 0;

      // Calculate rates
      const openRate =
        emailsDelivered > 0 ? (emailsOpened / emailsDelivered) * 100 : 0;
      const clickRate =
        emailsDelivered > 0 ? (emailsClicked / emailsDelivered) * 100 : 0;
      const bounceRate =
        ...
        campaigns?.map((campaign) => ({
          ...
          deliveryRate:
            campaign.sent_count > 0
              ? (
                  ((campaign.delivered_count || 0) / campaign.sent_count) *
                  100
                ).toFixed(2)
              : "0.00",
          openRate:
            campaign.delivered_count > 0
              ? (
                  ((campaign.opened_count || 0) / campaign.delivered_count) *
                  100
                ).toFixed(2)
              : "0.00",
          clickRate:
            campaign.delivered_count > 0
              ? (
                  ((campaign.clicked_count || 0) / campaign.delivered_count) *
                  100
                ).toFixed(2)
              : "0.00",
          bounceRate:
            campaign.sent_count > 0
              ? (
                  ((campaign.bounced_count || 0) / campaign.sent_count) *
                  100
                ).toFixed(2)
              : "0.00",
        })) || [];
